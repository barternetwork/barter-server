        ...
        await retry(
            async () => {
                ...
                while (true) {
                    ...
                    skip += PageSize
                    if (pools.length >= LiquidityMoreThan90Percent.UniSwap_V2 || poolsAtCurrentPage.length < PageSize) {
                        break
                    }
                }
                let data = {
                ...
                console.log(`query ${pools.length} uniswap v2 pools on chain ${this.chainId} costs:`, Date.now() - start);
